...
unsigned long Time;//
int heating;//needed for status update
...
//....................................................... smart commands........... 
      ...
      if(package.equals(on)){//checls if still on
         onOroff=1;
         stillon=1;
         XBEE.println("Water Heater Enabled");
         #ifdef DEBUG
           DEBUG_OUTPUT.println("Water Heater Enabled");
         #endif
       }
      if(package.equals(off)){
         onOroff=0;
         stillon=0;
         XBEE.println("Water Heater Disabled");
         #ifdef DEBUG
           DEBUG_OUTPUT.println("Water Heater Disabled");
         #endif
       }
      if(package.equals(temp)){           
       ...
       if(package.equals(stat)){
         ...
         //if wanting to add to the ifdef 
          #ifdef DEBUG
    
            DEBUG_OUTPUT.print("Temp: ");DEBUG_OUTPUT.print(temperature);DEBUG_OUTPUT.println(" C");
            if(stillon == 1){
              DEBUG_OUTPUT.print("Water Heater Enabled");
              
            }
            else{
              DEBUG_OUTPUT.print("Water Heater Disabled");
...
//............end of command code................. 
//..........CODE FOR MONITORING ON/OFF.........................    
   
     float thresholdup= SetPoint + deviationup;
     float thresholddown= SetPoint - deviationdown;
     /// code belows simply switches the on/off of the heater so that it does not over heat
     unsigned long TimerOne=millis();

     if ((TimerOne-state_time) <1000){
         state= HIGH;
         Serial.println("HIGH");//checking
     }
     else if(((TimerOne- state_time) >= 1000) && ((TimerOne-state_time) <2000)){
         state= LOW;
         Serial.println("LOW");// checking
     ...
     if(temperature <= thresholddown && onOroff == 1 ){//LED turns on if temperature is under setpoint and On is enabled from the XCTU menu.
         ...
         heating = 1;// tells status it is heating
      ...
      if(temperature >= thresholdup || onOroff == 0){//LED turns off if Off is enabled from the XCTU menu.
          ...
          heating=0;// tells status it is cooling
